...
filePath <- "C:/Users/User/Desktop/JobSeeking/Project/Forage/Quantium/task2/"
...
data$YEARMONTH <- as.integer(data$YEARMONTH)
...
ggplot(pastCustomers, aes(TransactionMonth, numberCustomers, color = Store_type)) +
  geom_line() +
  labs(x = "Month of operation", y ="The number of customers",
       title = "The number of customers by month")
```
...
percentageDiff <- merge(scaledControlCustomers[,c("YEARMONTH","controlCustomers")],
                        ...
                        by = "YEARMONTH"
                        )[, percentageDiff := abs(controlCustomers-nCustomers)/
                            controlCustomers]
```
...
score_nSales <- merge(corr_nSales, magnitude_nSales, 
                      by = c("Store1","Store2"))[,scoreNSales := (corr_measure +
                                                                    mag_measure)/2]
score_nCustomers <- merge(corr_nCustomers, magnitude_nCustomers,
...
scalingFactorForControlSales <- preTrialMeasures[STORE_NBR == trial_store &
                                                 ...
                                                 sum(totSales)]/preTrialMeasures[
                                                   STORE_NBR == control_store & YEARMONTH < 
                                                     201902, 
                                                   sum(totSales)
                                                   ]
#### Apply the scaling factor
...
## Trial store 88
```{r}
#### Your manager has left for a conference call, so you'll be
...
corr_weight <- 0.5
score_nSales <- merge(corr_nSales, magnitude_nSales, by = c("Store1", "Store2")
                      )[, scoreNSales := (corr_measure + mag_measure)/2]
...
control_store <- score_Control[Store1 == trial_store, ][order(-finalControlScore)
                                                        ][2, Store2]
control_store
```
...
ggplot(trialAssessment, aes(TransactionMonth, nCusts, color = Store_type)) +
  geom_rect(data = trialAssessment[YEARMONTH < 201905 & YEARMONTH > 201901,],
            aes(xmin = min(TransactionMonth), xmax = max(TransactionMonth),
                ymin = 0, ymax = Inf, col = NULL), show.legend = FALSE) +
  geom_line() +
  labs(x = "Month of operation", y = "Total number of customers",
       title = "Total number of customers by month")
```
